...
1. **파일 열기**
   - `lib/ch02_제어문/main.dart` 파일을 VS Code에서 엽니다

2. **실행 방법**
   - 방법 A: 파일 상단의 "Run" 버튼 클릭
   - 방법 B: `F5` 키 누르기
   - 방법 C: 우클릭 → "Run Dart" 선택
   - 방법 D: 터미널에서 `dart run lib/ch02_제어문/main.dart` 입력
...
1. **파일 열기**
   - `lib/ch02_제어문/main.dart` 파일을 엽니다
...
```bash
# 웹 브라우저에서 실행
flutter run -d chrome lib/ch02_제어문/main.dart

# 또는 특정 디바이스 선택
flutter devices  # 사용 가능한 디바이스 확인
flutter run -d <device-id> lib/ch02_제어문/main.dart
```
